<template>
  <div class="container">
    ...
    <div class="result-div">
      ...
      <scroll-view
        :style="{width:'99%','height':'100%','white-space':'nowrap','margin':'0 auto','flex':'10'}"
        scroll-x="true"
      >
        <div class="result" :style="{width:102+(result.length-1)*20+'rpx'}">
          <image
            :class="{pocker:true,active:item.active}"
            @click="backHandler($event,index,item.rowIndex,item.columnIndex)"
            :style="{right:(result.length-index)*20+'rpx'}"
    ...
    <div class="list">
      <div class="row" v-for="(item,index) in pocker" :key="index">
        <image
          @click="selectPocker($event,index,_index)"
          @touchstart="touchstart"
          @touchmove="touchmove"
          @touchend="touchend($event,index,_index)"
          ref="pocker"
          :class="{pocker:true,hidden:!_item.show,active:_item.active}"
...
export default {
  ...
  methods: {
    ...
    longPress: function (e, index, _index) {
      ...
      //index:行标
      //_index:列标
      this.selectBottomPocker = { e, index, _index };
    ...
    backHandler: function (e, index, row, column) {
      ...
      if (currentTime - this.lastClick < 300) {
        ...
        this.selectTopPocker = { e, index, row, column }
      }
    },
    hidden: function () {
    ...
    insertBefore: function () {
      ...
      this.$set(this.selectTopPocker, "index", this.result.filter(e => { return e.active })[0].index);
      this.hidden();
      this.show();
    },
